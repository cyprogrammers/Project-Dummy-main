...
export default function ComplianceOfficerPage({ onLogout, currentUser }) {
    ...
    const fetchTodayGdpr = async () => {
      ...
      try {
        const res = await fetch(
          `/api/v1/gdpr/evaluate?since=${encodeURIComponent(localMidnight())}`,
          { method: "POST" },
        );
  ...
  useEffect(() => {
    const fetchTodayPopia = async () => {
      ...
      try {
        const res = await fetch(
          `/api/v1/popia/evaluate?since=${encodeURIComponent(localMidnight())}`,
          { method: "POST" },
        );
        if (res.ok) setPopiaOverview(await res.json());
      } catch {
      } finally {
        setPopiaOverviewLoading(false);
      }
    };
    fetchTodayPopia();
  }, []);
  ...
  const runGdprEvaluation = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/v1/gdpr/evaluate?since=${encodeURIComponent(getSince(gdprWindow))}`,
        { method: "POST" },
      );
      if (!res.ok) {
  ...
  const runPopiaEvaluation = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/v1/popia/evaluate?since=${encodeURIComponent(getSince(popiaWindow))}`,
        { method: "POST" },
      );
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
  ...
  useEffect(() => {
    const fetchTodayIso = async () => {
      ...
      try {
        const res = await fetch(
          `/api/v1/iso27001/evaluate?since=${encodeURIComponent(localMidnight())}`,
          { method: "POST" },
        );
        if (res.ok) setIsoOverview(await res.json());
      } catch {
      } finally {
        setIsoOverviewLoading(false);
      }
    };
    fetchTodayIso();
  }, []);
  ...
  const runIsoEvaluation = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/v1/iso27001/evaluate?since=${encodeURIComponent(getSince(isoWindow))}`,
        { method: "POST" },
      );
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        ...
        {activePage === "overview" && (
          <>
                  ...
                  const overviewData = isGdpr
                    ...
                    : isPopia
                      ...
                      : isIso
                        ...
                        : null;
                  const loading =
                    (isGdpr && gdprOverviewLoading) ||
                    (isPopia && popiaOverviewLoading) ||
                    (isIso && isoOverviewLoading);
                  const score = overviewData
                    ? overviewData.overall_score
                    : null;
                  const findings = overviewData
                    ? (overviewData.findings ?? [])
                    : null;
                  const openFindings =
                    findings !== null ? findings.length : null;
                  const highCount =
                    findings !== null
                      ? findings.filter((f) => f.risk_level === "HIGH").length
                      : null;
                  const criticalCount =
                    findings !== null
                      ? findings.filter((f) => f.risk_level === "CRITICAL")
                          .length
                      : null;
                    ...
                    <div key={id} style={styles.frameworkCard}>
                      ...
                      <div
                        style={{
                          ...
                          background: dm ? "#334155" : trackColor,
                        }}
                      >
                        <div
                          style={{
                        ...
                        {hasCritical && (
                          <span style={styles.criticalBadge}>
                            ...
                            >
                              {loading
                                ? "…"
                                : `${highCount} high, ${criticalCount} critical`}
                            </span>
                          </span>
                        )}
                      </div>
            ...
            <div style={styles.chartCard}>
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                ...
                >
                  ...
                  {[
                    {
                      r: 30,
                      pts: "300,230 326,245 326,275 300,290 274,275 274,245",
                    },
                    {
                      r: 60,
                      pts: "300,200 352,230 352,290 300,320 248,290 248,230",
                    },
                    {
                      r: 90,
                      pts: "300,170 378,215 378,305 300,350 222,305 222,215",
                    },
                    {
                      r: 120,
                      pts: "300,140 404,200 404,320 300,380 196,320 196,200",
                    },
                    {
                      r: 150,
                      pts: "300,110 430,185 430,335 300,410 170,335 170,185",
                    },
                  ].map(({ r, pts }) => (
                    <polygon
                      key={r}
                      points={pts}
                      fill={dm ? "rgba(51,65,85,0.3)" : "rgba(243,244,246,0.6)"}
                      stroke={dm ? "#334155" : "#d1d5db"}
                      strokeWidth="1"
                    />
                  ))}
                  ...
                  {/* GDPR — purple */}
                  <polygon
                    points="300,260 300,260 300,260 300,260 300,260 300,260"
              ...
              >
                {[
                  { color: "#7c3aed", label: "GDPR" },
                  { color: "#16a34a", label: "POPIA" },
                  { color: "#d97706", label: "ISO 27001" },
                ].map(({ color, label }) => (
                  <div
                    key={label}
                    style={{
          ...
          >
                ...
                {gdprLlmStatus && (
                  <span
                    style={{
                      ...
                      fontSize: "12px",
                      fontWeight: "600",
                      background: gdprLlmStatus.llm_available
                        ? "#d1fae5"
                        : "#fef3c7",
                      color: gdprLlmStatus.llm_available
                        ? "#065f46"
                        : "#92400e",
                      border: `1px solid ${gdprLlmStatus.llm_available ? "#10b981" : "#f59e0b"}`,
                    }}
                  >
                    <span
                      style={{
                    ...
                    />
                    {gdprLlmStatus.llm_available
                      ? `Ollama · ${gdprLlmStatus.model}`
                      : "Rule-based fallback"}
                  </span>
                )}
              </div>
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "8px",
                    border: "none",
                    background: gdprLoading ? "#94a3b8" : "#7c3aed",
                    color: "white",
                    fontSize: "13px",
                    fontWeight: "700",
            ...
            {gdprLoading && (
              ...
              >
                ...
                >
                  AI is analysing system logs against GDPR…
                </div>
                <div
                  style={{
                    fontSize: "12px",
                ...
                >
                  This may take 15–60 seconds depending on the Ollama model
                </div>
              </div>
            )}
            ...
            {gdprReport &&
              ...
              (() => {
                const rs =
                  RISK_STYLE[gdprReport.risk_level] || RISK_STYLE.MEDIUM;
                const scoreColor =
                  gdprReport.overall_score >= 80
                    ? "#16a34a"
                    : gdprReport.overall_score >= 60
                      ? "#d97706"
                      : "#dc2626";
                return (
                  <>
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: "200px" }}>
                          ...
                          {[
                            {
                              ...
                              value: gdprReport.events_analyzed,
                            },
                            {
                              label: "Window",
                              value: `${gdprReport.log_window_hours}h`,
                            },
                            {
                              label: "Findings",
                              value: gdprReport.findings?.length ?? 0,
                            },
                        ...
                        >
                          Evaluated:{" "}
                          {new Date(gdprReport.evaluated_at).toLocaleString()}
                        </div>
                      </div>
                    </div>
                    ...
                    {gdprReport.findings && gdprReport.findings.length > 0 && (
                      <div style={styles.chartCard}>
                        ...
                        >
                          GDPR FINDINGS — {gdprReport.findings.length}{" "}
                          ARTICLE(S) ASSESSED
                        </div>
                        <div
                          style={{
                        ...
                        >
                          {gdprReport.findings.map((f, i) => {
                            const fs =
                              RISK_STYLE[f.risk_level] || RISK_STYLE.MEDIUM;
                            return (
                              <div
                                key={i}
                                style={{
            ...
            {!gdprReport && !gdprLoading && !gdprError && (
              ...
              >
                ...
                <div
                  style={{
                    fontSize: "13px",
                    color: dm ? "#475569" : "#d1d5db",
                    maxWidth: "380px",
                  }}
                >
                  Click <strong>Run GDPR Evaluation</strong> to analyse your
                  system activity logs against GDPR Articles 5, 17, 25, 32, and
                  33 using the Ollama AI model.
                </div>
              </div>
            )}
          ...
          >
                ...
                {popiaLlmStatus && (
                  <span
                    style={{
                      ...
                      fontSize: "12px",
                      fontWeight: "600",
                      background: popiaLlmStatus.llm_available
                        ? "#d1fae5"
                        : "#fef3c7",
                      color: popiaLlmStatus.llm_available
                        ? "#065f46"
                        : "#92400e",
                      border: `1px solid ${popiaLlmStatus.llm_available ? "#10b981" : "#f59e0b"}`,
                    }}
                  >
                    <span
                      style={{
                    ...
                    />
                    {popiaLlmStatus.llm_available
                      ? `Ollama · ${popiaLlmStatus.model}`
                      : "Rule-based fallback"}
                  </span>
                )}
              </div>
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "8px",
                    border: "none",
                    background: popiaLoading ? "#94a3b8" : "#16a34a",
                    color: "white",
                    fontSize: "13px",
                    fontWeight: "700",
                    cursor: popiaLoading ? "not-allowed" : "pointer",
                  }}
                >
                  {popiaLoading ? (
                    <>
            ...
            {popiaLoading && (
              ...
              >
                ...
                >
                  AI is analysing system logs against POPIA…
                </div>
                <div
                  style={{
                    fontSize: "12px",
            ...
            {popiaReport &&
              ...
              (() => {
                const rs =
                  RISK_STYLE[popiaReport.risk_level] || RISK_STYLE.MEDIUM;
                const scoreColor =
                  popiaReport.overall_score >= 80
                    ? "#16a34a"
                    : popiaReport.overall_score >= 60
                      ? "#d97706"
                      : "#dc2626";
                return (
                  <>
                      ...
                      <div style={{ flex: 1, minWidth: "200px" }}>
                        ...
                        >
                          {[
                            {
                              label: "Events Analysed",
                              value: popiaReport.events_analyzed,
                            },
                            {
                              label: "Window",
                              value: `${popiaReport.log_window_hours}h`,
                            },
                            {
                              label: "Findings",
                              value: popiaReport.findings?.length ?? 0,
                            },
                        ...
                        >
                          Evaluated:{" "}
                          {new Date(popiaReport.evaluated_at).toLocaleString()}
                        </div>
                      </div>
                    </div>

                    {/* Findings list */}
                    {popiaReport.findings &&
                      popiaReport.findings.length > 0 && (
                        <div style={styles.chartCard}>
                          <div
                            style={{
                              fontSize: "11px",
                          ...
                          >
                            POPIA FINDINGS — {popiaReport.findings.length}{" "}
                            CONDITION(S) ASSESSED
                          </div>
                          <div
                            style={{
                              display: "flex",
            ...
            {!popiaReport && !popiaLoading && !popiaError && (
              ...
              >
                ...
                >
                  Click <strong>Run POPIA Evaluation</strong> to analyse your
                  system activity logs against POPIA Act 4 of 2013 — Conditions
                  1, 2, 3, 7, and 8 — using the Ollama AI model.
                </div>
              </div>
            )}
          ...
          >
                ...
                {isoLlmStatus && (
                  <span
                    style={{
                      ...
                      fontSize: "12px",
                      fontWeight: "600",
                      background: isoLlmStatus.llm_available
                        ? "#d1fae5"
                        : "#fef3c7",
                      color: isoLlmStatus.llm_available ? "#065f46" : "#92400e",
                      border: `1px solid ${isoLlmStatus.llm_available ? "#10b981" : "#f59e0b"}`,
                    }}
                  >
                    <span
                      style={{
                    ...
                    />
                    {isoLlmStatus.llm_available
                      ? `Ollama · ${isoLlmStatus.model}`
                      : "Rule-based fallback"}
                  </span>
                )}
              </div>
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "8px",
                    border: "none",
                    background: isoLoading ? "#94a3b8" : "#d97706",
                    color: "white",
                    fontSize: "13px",
                    fontWeight: "700",
                    cursor: isoLoading ? "not-allowed" : "pointer",
                  }}
                >
                  {isoLoading ? (
                    <>
            ...
            {isoLoading && (
              ...
              >
                ...
                >
                  AI is analysing system logs against ISO 27001 Annex A…
                </div>
                <div
                  style={{
                    fontSize: "12px",
            ...
            {isoReport &&
              ...
              (() => {
                const rs =
                  RISK_STYLE[isoReport.risk_level] || RISK_STYLE.MEDIUM;
                const scoreColor =
                  isoReport.overall_score >= 80
                    ? "#16a34a"
                    : isoReport.overall_score >= 60
                      ? "#d97706"
                      : "#dc2626";
                return (
                  <>
                      ...
                      <div style={{ flex: 1, minWidth: "200px" }}>
                        ...
                        >
                          {[
                            {
                              label: "Events Analysed",
                              value: isoReport.events_analyzed,
                            },
                            {
                              label: "Window",
                              value: `${isoReport.log_window_hours}h`,
                            },
                            {
                              label: "Findings",
                              value: isoReport.findings?.length ?? 0,
                            },
                        ...
                        >
                          Evaluated:{" "}
                          {new Date(isoReport.evaluated_at).toLocaleString()}
                        </div>
                      </div>
                    </div>
                    ...
                    {isoReport.findings && isoReport.findings.length > 0 && (
                      <div style={styles.chartCard}>
                        ...
                        >
                          ISO 27001 FINDINGS — {isoReport.findings.length}{" "}
                          CONTROL(S) ASSESSED
                        </div>
                        <div
                          style={{
            ...
            {!isoReport && !isoLoading && !isoError && (
              ...
              >
                ...
                >
                  Click <strong>Run ISO 27001 Evaluation</strong> to analyse
                  your system activity logs against ISO/IEC 27001:2022 Annex A
                  controls — A.9 Access Control, A.12.4 Logging, A.16 Incident
                  Management, and more — using the Ollama AI model.
                </div>
              </div>
            )}
          ...
          (() => {
            const POLICY_RULES = [
              {
                ...
                trigger:
                  "More than 5 failed login attempts per user within a 1-hour window",
              ...
              },
              {
                ...
                trigger:
                  "Personal data accessed without a corresponding audit trail entry",
                action:
                  "Flag event as HIGH finding and notify Compliance Officer",
                severity: "HIGH",
                status: "Active",
              },
              {
                ...
                trigger:
                  "Security incident not reported to supervisory authority within 72 hours (Art. 33)",
                action:
                  "Escalate to CRITICAL and generate mandatory breach report",
                severity: "CRITICAL",
                status: "Active",
              },
              {
                ...
                trigger:
                  "User action on personal data with no matching consent log entry (Art. 7)",
              ...
              },
              {
                ...
                trigger:
                  "Personal information accessed outside 06:00–18:00 SAST (Condition 7)",
                action:
                  "Log MEDIUM finding and send alert to data protection officer",
                severity: "MEDIUM",
                status: "Active",
              },
              {
                ...
                trigger:
                  "Data transfer detected to a destination outside Zimbabwe (Condition 8)",
                action:
                  "Flag as HIGH finding and require operator justification",
                severity: "HIGH",
                status: "Active",
              },
              {
                ...
                trigger:
                  "API response returns fields beyond those necessary for the declared purpose (Condition 3)",
              ...
              },
              {
                ...
                trigger:
                  "Records held beyond the defined retention period (Condition 5)",
              ...
              },
              {
                ...
                trigger:
                  "Administrator session remains active for more than 8 consecutive hours (A.9)",
              ...
              },
              {
                ...
                trigger:
                  "Security incident not captured in the incident register within 72 hours (A.16)",
              ...
              },
              {
                ...
                trigger:
                  "Sensitive data transmission detected without TLS/encryption (A.10)",
              ...
              },
              {
                ...
                trigger:
                  "Privileged account not subject to access review within 30 days (A.9)",
            ...
            const sevStyle = (sev) => {
              ...
              return {
                ...
                color: "#059669",
                background: dm ? "transparent" : "#ecfdf5",
              };
            };

            const activeCount = POLICY_RULES.filter(
              (r) => r.status === "Active",
            ).length;
            const reviewCount = POLICY_RULES.filter(
            ...
            return (
              ...
              >
                ...
                <div style={styles.chartCard}>
                  ...
                  >
                    MACHINE-EXECUTABLE COMPLIANCE RULES — {POLICY_RULES.length}{" "}
                    RULES LOADED
                  </div>
                      ...
                      <tbody>
                        {POLICY_RULES.map((rule) => (
                          ...
                          >
                            ...
                            <td style={{ ...styles.td, whiteSpace: "nowrap" }}>
                              <span style={sevStyle(rule.severity)}>
                                {rule.severity}
                              </span>
                            </td>
                            <td style={{ ...styles.td, whiteSpace: "nowrap" }}>
...
const makeStyles = (dm) => ({
  ...
  findingCritical: {
    ...
    border: "1px solid #fca5a5",
    color: "#ef4444",
    background: dm ? "transparent" : "#fff5f5",
  },
  findingHigh: {
    display: "inline-block",
    padding: "4px 14px",
    ...
    border: "1px solid #fcd34d",
    color: "#d97706",
    background: dm ? "transparent" : "#fffbeb",
  },
  findingMedium: {
    display: "inline-block",
    padding: "4px 14px",
    ...
    border: "1px solid #fde68a",
    color: "#b45309",
    background: dm ? "transparent" : "#fefce8",
  },
  fwGdpr: {
    display: "inline-block",
    padding: "3px 12px",
