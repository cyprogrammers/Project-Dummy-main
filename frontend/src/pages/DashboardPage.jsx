...
function RoleProfileView({ role, onBack, dm, styles }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setUserPermissions(
          rolePermissions.map((p) => ({ ...p, is_override: false })),
        );
      } finally {
    ...
    setUserPermissions((prev) =>
      prev.map((p) =>
        p.module_id === moduleId
          ? { ...p, [`can_${permType}`]: !p[`can_${permType}`] }
          : p,
      ),
    );
  };
  ...
  const handleSaveUserPermissions = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Permission overrides could not be saved:",
          permErr.message,
        );
      }
      ...
      if (!hasAnyRead && currentStatus === "active") {
        await usersAPI.setStatus(
          selectedPermUser.id,
          "suspended",
          "All module read permissions revoked",
        );
        setUsers((prev) =>
          prev.map((u) =>
            u.id === selectedPermUser.id ? { ...u, status: "suspended" } : u,
          ),
        );
        setSelectedPermUser((prev) => ({ ...prev, status: "suspended" }));
        alert(
          `${selectedPermUser.first_name} ${selectedPermUser.surname} has been SUSPENDED — no read access granted.`,
        );
      } else if (hasAnyRead && currentStatus === "suspended") {
        await usersAPI.setStatus(
          selectedPermUser.id,
          "active",
          "Module read permissions restored",
        );
        setUsers((prev) =>
          prev.map((u) =>
            u.id === selectedPermUser.id ? { ...u, status: "active" } : u,
          ),
        );
        setSelectedPermUser((prev) => ({ ...prev, status: "active" }));
        alert(
          `${selectedPermUser.first_name} ${selectedPermUser.surname} has been REACTIVATED — read access restored.`,
        );
      } else {
        alert(
          `Permissions updated for ${selectedPermUser.first_name} ${selectedPermUser.surname}.`,
        );
      }
  ...
  const handleUpdateUser = async () => {
    ...
    if (editOldPassword || editNewPassword || editConfirmPassword) {
      if (!editOldPassword) {
        setEditPwError("Please enter your current password.");
        return;
      }
      if (!editNewPassword) {
        setEditPwError("Please enter a new password.");
        return;
      }
      ...
      if (editNewPassword !== editConfirmPassword) {
        setEditPwError("New passwords do not match.");
        return;
      }
    ...
    try {
      const updates = {};
      if (editFirstName && editFirstName !== editingUser.first_name)
        updates.first_name = editFirstName;
      if (editSurname && editSurname !== editingUser.surname)
        updates.surname = editSurname;
      ...
      if (editOldPassword && editNewPassword) {
        try {
          ...
          setEditPwError(
            pwErr.message?.includes("400") ||
              pwErr.message?.toLowerCase().includes("incorrect")
  ...
  const handleSaveSettings = async () => {
    ...
    const attemptsVal =
      maxFailedAttempts !== "" ? parseInt(maxFailedAttempts) : null;

    if (
      timeoutVal !== null &&
      (isNaN(timeoutVal) || timeoutVal < 1 || timeoutVal > 1440)
    ) {
    ...
    if (
      attemptsVal !== null &&
      (isNaN(attemptsVal) || attemptsVal < 1 || attemptsVal > 20)
    ) {
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      ...
      await rolesAPI.updatePerms(roleData.id, {
          ...
          <div>
            ...
            >
              ...
              <span style={{ color: "#f97316" }}>
                {users.filter((u) => u.mfa_enabled).length} MFA enabled
              </span>
            </div>
          </div>
        </div>
          ...
          <div style={{ padding: "20px 24px" }}>
            ...
            {selectedPermUser ? (
              (() => {
                ...
                const handleRevokeAccess = async () => {
                  if (
                    !window.confirm(
                      `Suspend ${selectedPermUser.first_name} ${selectedPermUser.surname}?\n\nThey will be blocked from logging in immediately.`,
                    )
                  )
                    return;
                  try {
                    await usersAPI.setStatus(
                      selectedPermUser.id,
                      "suspended",
                      "Access revoked from Permissions tab",
                    );
                    setUsers((prev) =>
                      prev.map((u) =>
                        u.id === selectedPermUser.id
                          ? { ...u, status: "suspended" }
                          : u,
                      ),
                    );
                    setSelectedPermUser((prev) => ({
                      ...prev,
                      status: "suspended",
                    }));
                    alert(
                      `${selectedPermUser.first_name} ${selectedPermUser.surname} suspended — login blocked.`,
                    );
                  } catch (err) {
                    alert("Failed: " + err.message);
                  }
                ...
                const handleRestoreAccess = async () => {
                  try {
                    await usersAPI.setStatus(
                      selectedPermUser.id,
                      "active",
                      "Access restored from Permissions tab",
                    );
                    setUsers((prev) =>
                      prev.map((u) =>
                        u.id === selectedPermUser.id
                    ...
                    alert(
                      `${selectedPermUser.first_name} ${selectedPermUser.surname} reactivated.`,
                    );
                  } catch (err) {
                    alert("Failed: " + err.message);
                  }
                  ...
                  <>
                    ...
                    <div
                      style={{
                        ...
                        border: `1px solid ${isSuspended ? "#ef4444" : dm ? "#334155" : "#e5e7eb"}`,
                        borderRadius: "12px",
                        padding: "14px 18px",
                        marginBottom: "16px",
                      }}
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          {selectedPermUser.first_name}{" "}
                          {selectedPermUser.surname}
                        </div>
                        <div
                          style={{
                            fontSize: "12px",
                    ...
                    >
                      <thead>
                        ...
                        >
                          {["Module", "Read", "Write", "Delete", "Admin"].map(
                            (col) => (
                              <th
                                key={col}
                                style={{
                        ...
                        {userPermsLoading ? (
                          ...
                          userPermissions.map((perm, i) => (
                            <tr
                              ...
                              style={{
                                borderBottom:
                                  i < userPermissions.length - 1
                                    ? `1px solid ${dm ? "#1e293b" : "#f3f4f6"}`
                                    : "none",
                              }}
                            >
                              <td
              ...
              <div
                style={{
                  ...
                  color: dm ? "#64748b" : "#9ca3af",
                  fontSize: "14px",
                  border: `1px dashed ${dm ? "#334155" : "#e5e7eb"}`,
                  borderRadius: "12px",
                  marginBottom: "20px",
                }}
              >
            ...
            <div
              style={{
                ...
                border: `1px solid ${dm ? "#78350f" : "#fde68a"}`,
                borderRadius: "10px",
                padding: "14px 16px",
              }}
            >
              ...
              >
                Use <strong>Revoke Access</strong> to immediately block a user
                from logging in. Module permissions control what they can do
                once logged in. All changes are logged per ISO 27001 A.9.
              </span>
            </div>
          </div>
        )}
          ...
          <div style={{ padding: "24px" }}>
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      Require MFA for all users in this role
                    </div>
                    <div
                      style={{
                        fontSize: "12px",
                ...
                >
                  ...
                  <input
                    ...
                    style={{
                      ...
                      padding: "8px 12px",
                      borderRadius: "8px",
                      border: `1px solid ${dm ? "#475569" : "#d1d5db"}`,
                      background: dm ? "#0f172a" : "#f9fafb",
                      color: dm ? "#f1f5f9" : "#111827",
                      fontSize: "14px",
                      textAlign: "center",
                ...
                >
                  <div>
                    ...
                    >
                      Privileged Access Management (PAM) sessions
                    </div>
                    <div
                      style={{
                        fontSize: "12px",
                    ...
                    >
                      Record and monitor privileged sessions for this role
                    </div>
                  </div>
                  {/* PAM Toggle */}
                  <div
                    onClick={() => setPamEnabled((v) => !v)}
                    style={{
                      position: "relative",
                      width: "44px",
                      height: "24px",
            ...
            <div style={{ marginTop: "20px" }}>
              ...
              >
                {[
                  {
                    label: "Restrict access to working hours (08:00–17:00 CAT)",
                    val: restrictHours,
                    set: setRestrictHours,
                  },
                  {
                    label: "Geo-restriction (ZW only)",
                    val: geoRestrict,
                    set: setGeoRestrict,
                  },
                  {
                    label: "Allow concurrent sessions",
                    val: concurrentSessions,
                    set: setConcurrentSessions,
                  },
            ...
            <div style={{ marginTop: "20px" }}>
              ...
              >
                {[
                  {
                    label: "Email alert on new login",
                    val: emailAlertLogin,
                    set: setEmailAlertLogin,
                  },
                  {
                    label: "Alert on role assignment change",
                    val: alertRoleChange,
                    set: setAlertRoleChange,
                  },
                  {
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                ...
                >
                  {["Timestamp", "Actor", "Action", "Details", "Result"].map(
                    (col) => (
                      <th
                        key={col}
                        style={{
                ...
                {activityLoading ? (
                  ...
                  activityLogs.map((log, i, arr) => {
                    ...
                    if (
                      ...
                      log.target_user_email !== log.actor_email
                    ) {
                        ...
                        <td style={{ padding: "14px 20px" }}>
                          <span
                            style={{
                              ...
                              fontWeight: "700",
                              letterSpacing: "0.5px",
                              border: `1px solid ${dm ? "#0891b2" : "#67e8f9"}`,
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ padding: "18px 20px" }}>
                              <span
                                onClick={() =>
                                  handleToggleUserMFA(user.id, user.mfa_enabled)
                                }
                                style={{
                                  display: "inline-block",
                                  padding: "3px 14px",
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  {editingUser
                    ? `${editingUser.first_name?.[0] || ""}${editingUser.surname?.[0] || ""}`.toUpperCase()
                    : "—"}
                </span>
              </div>
              <div>
                ...
                >
                  {editingUser
                    ? `${editingUser.first_name || ""} ${editingUser.surname || ""}`.trim()
                    : "—"}
                </div>
                <div
                  style={{
            ...
            <div style={{ marginBottom: "18px" }}>
              ...
              >
                {editingUser?.status?.charAt(0).toUpperCase() +
                  editingUser?.status?.slice(1) || "Unknown"}{" "}
                (Use status toggle in table)
              </div>
            </div>
            ...
            <div style={{ marginBottom: "4px" }}>
              ...
              <div style={{ marginBottom: "6px" }}>
                ...
                <input
                  type={showEditPasswords ? "text" : "password"}
                  ...
                  onBlur={async () => {
                    ...
                    try {
                      const res = await usersAPI.verifyPassword(
                        editingUser.id,
                        editOldPassword,
                      );
                  ...
                  style={{
                    width: "100%",
                    padding: "12px 14px",
                    border: `1px solid ${editOldPasswordValid === false ? "#ef4444" : editOldPasswordValid === true ? "#16a34a" : dm ? "#334155" : "#e5e7eb"}`,
                    borderRadius: "10px",
                    fontSize: "14px",
                    color: dm ? "#f1f5f9" : "#111827",
                    background: dm ? "#0f172a" : "#f9fafb",
              ...
              >
                {editOldPasswordChecking && (
                  <span style={{ color: dm ? "#94a3b8" : "#6b7280" }}>
                    Verifying...
                  </span>
                )}
                {!editOldPasswordChecking && editOldPasswordValid === false && (
                  <span style={{ color: "#ef4444" }}>
                    Current password is incorrect.
                  </span>
                )}
                {!editOldPasswordChecking && editOldPasswordValid === true && (
                  <span style={{ color: "#16a34a" }}>Password verified.</span>
                )}
              </div>
              ...
              <div
                style={{
                  marginBottom: "14px",
                  opacity: editOldPasswordValid === true ? 1 : 0.4,
                  pointerEvents:
                    editOldPasswordValid === true ? "auto" : "none",
                }}
              >
                <label
                  style={{
              ...
              <div
                style={{
                  marginBottom: "10px",
                  opacity: editNewPassword ? 1 : 0.4,
                  pointerEvents: editNewPassword ? "auto" : "none",
                }}
              >
                <label
                  style={{
                ...
                <input
                  ...
                  style={{
                    width: "100%",
                    padding: "12px 14px",
                    border: `1px solid ${editPwError && editConfirmPassword && editConfirmPassword !== editNewPassword ? "#ef4444" : dm ? "#334155" : "#e5e7eb"}`,
                    borderRadius: "10px",
                    fontSize: "14px",
                    color: dm ? "#f1f5f9" : "#111827",
                    background: dm ? "#0f172a" : "#f9fafb",
              ...
              >
                <input
                  type="checkbox"
                  checked={showEditPasswords}
                  onChange={(e) => setShowEditPasswords(e.target.checked)}
                  style={{
                    width: "15px",
                    height: "15px",
                    cursor: "pointer",
          ...
          >
            ...
            <div style={{ textAlign: "center", marginBottom: "28px" }}>
              ...
              >
                ...
                <span style={{ color: "#f97316" }}>
                  {removingUser
                    ? `${removingUser.first_name || ""} ${removingUser.surname || ""}`.trim()
                    : "—"}
                </span>{" "}
                from {role}?
              </p>
            ...
            >
              ...
              <button
                onClick={async () => {
                  await handleDeleteUser(removingUser?.id);
                  setShowRemoveUser(false);
                }}
                style={{
                  padding: "10px 28px",
          ...
          >
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  style={{
                    width: "100%",
                    padding: "12px 14px",
                    border: `1px solid ${newUserConfirmPassword && newUserPassword !== newUserConfirmPassword ? "#ef4444" : dm ? "#334155" : "#e5e7eb"}`,
                    borderRadius: "10px",
                    fontSize: "14px",
                    color: dm ? "#f1f5f9" : "#111827",
                    background: dm ? "#0f172a" : "#f9fafb",
            ...
            >
              <div>
                ...
                >
                  Mandatory for IT Admin &amp; Security Analyst
                </div>
              </div>
              <button
...
export default function DashboardPage({ onLogout, currentUser }) {
  ...
  const RF_ROWS = [
    {
      label: "Critical",
      sublabel: "admin accounts, no MFA",
      color: "#ef4444",
      count: rfCritical,
    },
    {
      label: "High",
      sublabel: "blocked events today",
      color: "#f97316",
      count: rfHigh,
    },
    {
      label: "Medium",
      sublabel: "inactive active accounts",
      color: "#f59e0b",
      count: rfMedium,
    },
    {
      label: "Low",
      sublabel: "backup job failures",
      color: "#22c55e",
      count: rfLow,
    },
  ];
  ...
  const makeAreaPath = (values) => {
    const W = 800,
      H = 200;
    const lastX =
      values.length < 2 ? W : (values.length - 1) * (W / (values.length - 1));
    ...
    <div style={styles.wrapper}>
        ...
        <div style={styles.profile}>
          <div style={styles.profileInner}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              >
                {currentUser
                  ? `${(currentUser.first_name?.[0] || "").toUpperCase()}. ${currentUser.surname}`
                  : "Admin"}
              </div>
              <div
                style={{
      ...
      <main style={styles.main}>
          ...
          <>
            <div style={styles.cardGrid}>
              <div
                style={{
                  ...
                  border: `1px solid ${dm ? "#065f46" : "#dcfce3"}`,
                }}
              >
              ...
              <div
                style={{
                  ...styles.card,
                  background: dm ? "#451a03" : "#fffbeb",
                  border: `1px solid ${dm ? "#78350f" : "#fef3c7"}`,
                }}
              >
                ...
                >
                  {rbacStats
                    ? `${rbacStats.mfa_adoption_rate}% adoption rate`
                    : "—% adoption rate"}
                </div>
              </div>
              <div
              ...
              <div
                style={{
                  ...styles.card,
                  background: dm ? "#083344" : "#ecfeff",
                  border: `1px solid ${dm ? "#164e63" : "#cffafe"}`,
                }}
              >
            ...
            <div style={styles.chartCard}>
              <div style={styles.chartHeader}>
                <span style={styles.chartTitle}>ROLE PERMISSIONS MATRIX</span>
              </div>
              <table style={styles.table}>
                ...
                <tbody>
                  {[
                    ...
                    const activeCount = roleData
                      ? roleData.active_user_count
                      : "—";
                    return (
                      <tr key={role}>
                        <td
                        ...
                        <td style={styles.td}>
                          {mfa === "required" ? (
                            <span style={styles.mfaRequired}>Required</span>
                          ) : (
                            <span style={styles.mfaOptional}>Optional</span>
                          )}
                        </td>
                        <td style={styles.td}>
                          <button
        ...
        {activePage === "incidents" && (
          <div style={styles.chartCard}>
            <div style={styles.chartHeader}>
              <span style={styles.chartTitle}>OPEN &amp; ACTIVE INCIDENTS</span>
            </div>
            <div style={styles.incidentList}>
                      ...
                      <div style={styles.incidentLeft}>
                        ...
                        <div>
                          ...
                          <div style={styles.incidentMeta}>
                            {incident_id} &bull; {storyline_id} &bull; Assigned:{" "}
                            {assigned} &bull; Source: {source_ip || "unknown"}
                          </div>
                        </div>
                      </div>
              ...
              {(incidentData.incidents || []).length === 0 && (
                ...
                >
                  No incidents to display. Run an attack simulation to generate
                  test incidents.
                </div>
              )}
              {false &&
                [
                  {
                    title: "Brute-force on admin account",
                    id: "INC-0041",
                    assigned: "analyst_01",
                    dot: "#ef4444",
                    status: "open",
                  ...
                  {
                    title: "Email archive backup failure",
                    id: "INC-0040",
                    assigned: "tech_02",
                    dot: "#f97316",
                    status: "in-progress",
                  ...
                  {
                    title: "Unauthorised access /data/finance",
                    id: "INC-0039",
                    assigned: "analyst_02",
                    dot: "#ef4444",
                    status: "open",
                  ...
                  {
                    title: "GDPR control gap \u2014 Data Retention",
                    id: "INC-0037",
                    assigned: "compliance",
                    dot: "#f97316",
                    status: "in-progress",
                  ...
                  ({ title, id, assigned, dot, status, severity }, i, arr) => (
                    <div
                      key={id}
                      style={{
                        ...styles.incidentRow,
                    ...
                    >
                      <div style={styles.incidentLeft}>
                        <span
                          style={{ ...styles.incidentDot, background: dot }}
                        />
                        <div>
                          <div style={styles.incidentTitle}>{title}</div>
                          <div style={styles.incidentMeta}>
                            {id} &bull; Assigned: {assigned}
                          </div>
                        </div>
                      </div>
                      <div style={styles.incidentRight}>
        ...
        {activePage === "risk" && (
          <>
            <div style={styles.cardGrid}>
              ...
              >
                ...
                <div style={styles.cardValue}>
                  {riskData ? riskData.overall_risk_score.toFixed(1) : "—"}
                </div>
              ...
              >
                ...
                <div style={styles.cardValue}>
                  {riskData ? riskData.critical_vulns : "—"}
                </div>
                <div
                  style={{
                    ...styles.cardMeta,
              ...
              <div
                style={{
                  ...styles.card,
                  background: dm ? "#064e3b" : "#f0fdf4",
                  border: `1px solid ${dm ? "#065f46" : "#dcfce7"}`,
                }}
              >
                <div style={styles.cardHeader}>
                ...
                <div style={styles.cardValue}>
                  {riskData ? riskData.active_incidents : "—"}
                </div>
                <div
                  style={{
                    ...styles.cardMeta,
                ...
                >
                  {riskData
                    ? `${riskData.critical_incidents} critical, ${riskData.medium_incidents} medium`
                    : "— critical, — medium"}
                </div>
              </div>
              ...
              >
                ...
                <div style={styles.cardValue}>
                  {riskData ? riskData.monitored_endpoints : "—"}
                </div>
                <div
                  style={{
                    ...styles.cardMeta,
            ...
            <div style={styles.chartCard}>
                  ...
                  {[
                    {
                      color: "#3b82f6",
                      label: "Total Events",
                      count: trendTotalEvents,
                    },
                    {
                      color: "#ef4444",
                      label: "Blocked / Failed",
                      count: trendTotalBlocked,
                    },
                    {
                      color: "#f97316",
                      label: "Access Changes",
                      count: trendTotalAccess,
                    },
              ...
              <div style={styles.chartArea}>
                  ...
                  <div style={styles.yAxis}>
                    {(() => {
                      ...
                      return [top, step * 3, step * 2, step, 0].map((v, i) => (
                        <span key={i} style={styles.yLabel}>
                          {v}
                        </span>
                      ));
                    })()}
                  ...
                  <div style={styles.chartPlaceholder}>
                    {trendLoading && trendLogs.length === 0 ? (
                      ...
                      >
                        ...
                        <path
                          d={makeAreaPath(trendData.map((d) => d.total))}
                          fill="rgba(59,130,246,0.08)"
                        />
                        <path
                          d={makeTrendPath(trendData.map((d) => d.total))}
                          fill="none"
                          stroke="#3b82f6"
                          strokeWidth="2.5"
                          strokeLinecap="round"
                        ...
                        <path
                          d={makeTrendPath(trendData.map((d) => d.blocked))}
                          fill="none"
                          stroke="#ef4444"
                          strokeWidth="2"
                          strokeLinecap="round"
                        ...
                        <path
                          d={makeTrendPath(trendData.map((d) => d.access))}
                          fill="none"
                          stroke="#f97316"
                          strokeWidth="2"
                          strokeLinecap="round"
                        ...
                        {trendPointCoords(trendData.map((d) => d.total)).map(
                          (pt, i) => (
                            <circle
                              key={i}
                              cx={pt.x}
                ...
                <div style={styles.xAxis}>
                  {trendData.map((d) => (
                    <span key={d.label} style={styles.xLabel}>
                      {d.label}
                    </span>
                  ))}
                </div>
            ...
            <div style={styles.chartCard}>
              <div style={styles.chartHeader}>
                <span style={styles.chartTitle}>DEPARTMENT RISK HEATMAP</span>
              </div>
                ...
                <div style={styles.heatmapXAxis}>
                  <div style={styles.heatmapXLabels}>
                    {[0, 25, 50, 75, 100].map((v) => (
                      <span key={v} style={styles.heatmapXLabel}>
                        {v}
                      </span>
                    ))}
                  </div>
            ...
            <div style={styles.chartCard}>
              <div style={styles.chartHeader}>
                <span style={styles.chartTitle}>
                  SECURITY RISK FINDINGS BY SEVERITY
                </span>
                <span
                  style={{
                    fontSize: "11px",
                    ...
                    <div key={label} style={styles.vulnRow}>
                      <div style={styles.vulnRowHeader}>
                        <div style={styles.vulnRowLeft}>
                          <span
                            style={{ ...styles.vulnDot, background: color }}
                          />
                          <div>
              ...
              <div style={styles.vulnFooter}>
                ...
                <span
                  style={{
                    ...styles.vulnTotalValue,
                    color: rfTotal > 0 ? "#ef4444" : dm ? "#94a3b8" : "#6b7280",
                  }}
                >
                  {rfTotal} findings
                </span>
        ...
        {activePage === "tasks" && (
          <TaskManagementPanel darkMode={darkMode} currentUser={currentUser} />
        )}
...
const makeStyles = (dm) => ({
  ...
  sidebar: {
    ...
    height: "100vh",
    background: dm ? "#1e293b" : "white",
    borderRight: `1px solid ${dm ? "#334155" : "#bfdbfe"}`,
    boxShadow: dm ? "none" : "2px 0 12px rgba(37,99,235,0.06)",
    display: "flex",
    flexDirection: "column",
    padding: "32px 0 20px",
    flexShrink: 0,
  ...
  brandName: {
    fontSize: "24px",
    fontWeight: "800",
    color: dm ? "#f1f5f9" : "#1e3a5f",
    lineHeight: "1.2",
  },
  brandSub: {
    fontSize: "14px",
    color: dm ? "#64748b" : "#6b9dc0",
  },
  nav: {
    display: "flex",
    flexDirection: "column",
  ...
  profile: {
    ...
    gap: "10px",
    padding: "14px 20px",
    borderTop: `1px solid ${dm ? "#334155" : "#bfdbfe"}`,
    flexShrink: 0,
  },
  profileInner: {
    display: "flex",
  ...
  card: {
    ...
    border: `1px solid ${dm ? "#334155" : "#dbeafe"}`,
  },
  cardHeader: {
    display: "flex",
    alignItems: "center",
  ...
  legend: {
    ...
    marginTop: "20px",
    paddingTop: "16px",
    borderTop: `1px solid ${dm ? "#334155" : "#dbeafe"}`,
  },
  legendItem: {
    display: "flex",
    alignItems: "center",
  ...
  th: {
    ...
    fontSize: "12px",
    fontWeight: "700",
    color: dm ? "#94a3b8" : "#1e3a5f",
    padding: "14px 16px",
    textTransform: "uppercase",
    letterSpacing: "0.8px",
    borderBottom: `1px solid ${dm ? "#334155" : "#bfdbfe"}`,
  },
  td: {
    padding: "16px",
    fontSize: "14px",
    color: dm ? "#94a3b8" : "#374151",
    borderBottom: `1px solid ${dm ? "#334155" : "#dbeafe"}`,
  },
  mfaRequired: {
    display: "inline-block",
    padding: "3px 12px",
