...
const PAGE_HEADERS = {
  "audit-trail": {
    ...
    subtitle:
      "Tamper-Evident Audit Trail · Read-Only View · 12-Month Retention",
...
const EXPORT_TYPES = [
  {
    id: "audit-trail",
    label: "Audit Trail",
    desc: "All system events, logins & access changes",
  },
  {
    id: "control-effectiveness",
    label: "Control Effectiveness",
    desc: "GDPR / POPIA / ISO 27001 live control scores",
  },
  {
    id: "gdpr",
    label: "GDPR Evaluation",
    desc: "Full GDPR compliance findings & recommendations",
  },
  {
    id: "popia",
    label: "POPIA Evaluation",
    desc: "Full POPIA compliance findings & recommendations",
  },
  {
    id: "iso27001",
    label: "ISO 27001 Evaluation",
    desc: "ISO 27001 audit findings & observations",
  },
  {
    id: "backup-status",
    label: "Backup Status",
    desc: "Backup jobs, integrity checks & storage utilization",
  },
  {
    id: "task-report",
    label: "Task Report",
    desc: "IT tasks, assignments & completion status",
  },
  {
    id: "user-access",
    label: "User Access Report",
    desc: "Users, roles, permissions & MFA status",
  },
];
...
export default function SystemAuditorPage({ onLogout, currentUser }) {
  ...
  useEffect(() => {
    ...
    const fetchControls = async (showSpinner = false) => {
      ...
      try {
        ...
        if (active)
          setControlsError(err.message || "Failed to load control metrics");
      } finally {
  ...
  const filteredLogs = auditLogs.filter((log) => {
    ...
    if (appliedTo && ms > new Date(appliedTo + "T23:59:59.999Z").getTime())
      return false;
    return true;
  });
  ...
  const baseLogs = filterActive
    ? filteredLogs
    : auditLogs.filter((log) => isTodayLocal(log.timestamp));
  ...
  const applyFilter = () =>
    setAppliedFrom(filterFrom) || setAppliedTo(filterTo);
  const clearFilter = () => {
    setFilterFrom("");
    setFilterTo("");
    setAppliedFrom("");
  ...
  const buildExportHTML = (
    ...
    const period =
      from || to ? `${from || "start"} → ${to || "now"}` : "Full dataset";
...
<table><thead><tr>${thead}</tr></thead><tbody>${tbody}</tbody></table>
  ...
  const handleGenerateExport = async () => {
    ...
    try {
      ...
      const toMs = exportTo
        ? new Date(exportTo + "T23:59:59.999Z").getTime()
        : null;
      const inRange = (ts) => {
        if (!ts) return true;
        const t = new Date(ts).getTime();
      ...
      const byDate = (rows, field) =>
        fromMs || toMs ? rows.filter((r) => inRange(r[field])) : rows;
      ...
      if (exportType === "audit-trail") {
        ...
        while (true) {
          const batch = await authFetch(
            `/api/v1/rbac/activity?page_size=${PAGE}&page=${page}`,
          );
          if (!Array.isArray(batch) || batch.length === 0) break;
          all = [...all, ...batch];
          if (batch.length < PAGE) break;
        ...
        const urlMap = {
          gdpr: "/api/v1/gdpr/evaluate?hours=24",
          popia: "/api/v1/popia/evaluate?hours=24",
          iso27001: "/api/v1/iso27001/evaluate?hours=24",
        };
        ...
        htmlContent = buildExportHTML(
          `${nameMap[exportType]} Compliance Report`,
        ...
        const taskList = byDate(
          Array.isArray(tasks) ? tasks : tasks?.items || tasks?.tasks || [],
          "created_at",
        );
    ...
    <div style={styles.wrapper}>
      ...
      <main style={styles.main}>
        ...
        <div style={styles.cardGrid4}>
          ...
          >
            <span
              style={{ ...styles.cardLabel, color: dm ? "#94a3b8" : "#6b7280" }}
            >
              EVENTS (24H)
            </span>
            <div style={{ ...styles.cardValue, color: "#16a34a" }}>
              {auditLoading ? "…" : count24h}
            </div>
            <div
              style={{ fontSize: "13px", color: dm ? "#94a3b8" : "#6b7280" }}
            >
              {lastRefreshed
          ...
          >
            ...
            <div style={{ ...styles.cardValue, color: "#0ea5e9" }}>
              {auditLoading ? "…" : accessChangesCount}
            </div>
            <div
              style={{ fontSize: "13px", color: dm ? "#94a3b8" : "#6b7280" }}
            >
              Role, account & auth changes
            </div>
          </div>
          <div
            style={{
                ...
                style={{
                  ...
                  background: showFilter
                    ? dm
                      ...
                      : "#eff6ff"
                    : undefined,
                  borderColor: showFilter ? "#3b82f6" : undefined,
                  color: showFilter ? "#3b82f6" : undefined,
                }}
              >
                <svg
                  width="14"
          ...
          {activePage === "audit-trail" && showFilter && (
            ...
            >
              ...
              {(appliedFrom || appliedTo) && (
                ...
                >
                  Showing {baseLogs.length} of {auditLogs.length} events
                </span>
              )}
            </div>
          )}
          ...
          {activePage === "audit-trail" ? (
            <div style={styles.tableWrapper}>
              <table style={styles.table}>
                <thead>
                  <tr>
                    {["TIMESTAMP", "ACTOR", "ACTION", "TARGET", "RESULT"].map(
                      (col) => (
                  ...
                  {auditLoading ? (
                    ...
                    baseLogs.map((log, i, arr) => {
                      ...
                      if (action === "MFA_TOGGLE") {
                        ...
                        targetLabel = {
                          text:
                            fieldText.charAt(0).toUpperCase() +
                            fieldText.slice(1) +
                            " updated",
                          color: "#1d4ed8",
                          bg: "#dbeafe",
                        ...
                        targetLabel = log.role_name
                          ? {
                              text: `Assigned: ${log.role_name}`,
                              color: "#7c3aed",
                              bg: "#ede9fe",
                            }
                          : null;
            ...
            (() => {
              ...
              const passingCount = metrics.filter(
                (c) => c.status === "Passing",
              ).length;
              const degradedCount = metrics.filter(
                (c) => c.status === "Degraded",
              ).length;
              const failingCount = metrics.filter(
                (c) => c.status === "Failing",
              ).length;
              const avgEff =
                metrics.length > 0
                  ? Math.round(
                      metrics.reduce((s, c) => s + c.effectiveness, 0) /
                        metrics.length,
                    )
                  : 0;
                  ...
                  >
                    ...
                    {controlsData && (
                      ...
                      >
                        Updated{" "}
                        {new Date(
                          controlsData.assessed_at,
                        ).toLocaleTimeString()}{" "}
                        · {controlsData.window_hours}h window
                      </span>
                    )}
                  </div>
                              ...
                              >
                                <span
                                  style={{
                                    ...
                                    border: `1px solid ${dm ? "#1d4ed8" : "#bfdbfe"}`,
                                  }}
                                >
                                  {c.domain}
                                </span>
            ...
            >
              ...
              >
                ...
                <div
                  style={{
                    display: "grid",
                    gridTemplateColumns: "2fr 120px 140px 140px auto",
                    gap: "12px",
                    alignItems: "end",
                  }}
                >
              ...
              {exportHistory.length === 0 ? (
                ...
                >
                  No exports generated yet. Use the panel above to create your
                  first report.
                </div>
              ) : (
                <div style={{ overflowX: "auto" }}>
                        ...
                        const fmtStyle =
                          entry.format === "CSV"
                            ? {
                                bg: dm ? "#1e3a5f" : "#eff6ff",
                                color: dm ? "#93c5fd" : "#1d4ed8",
                                border: dm ? "#1d4ed8" : "#bfdbfe",
                              }
                            : {
                                bg: dm ? "#2e1065" : "#f5f3ff",
                                color: dm ? "#c4b5fd" : "#7c3aed",
                                border: dm ? "#7c3aed" : "#ddd6fe",
                              };
                        return (
                          <tr
                            key={entry.id}
                          ...
                          >
                            ...
                            >
                              {entry.status === "generating"
                                ? "..."
                                : (entry.records ?? 0).toLocaleString()}
                            </td>
                            <td style={styles.td}>
                              <span
                                style={{
...
const makeStyles = (dm) => ({
  ...
  th: {
    ...
    letterSpacing: "0.8px",
    textTransform: "uppercase",
    color: dm ? "#64748b" : "#1e3a5f",
    borderBottom: `2px solid ${dm ? "#334155" : "#bfdbfe"}`,
    whiteSpace: "nowrap",
  },
  td: {
    padding: "16px",
